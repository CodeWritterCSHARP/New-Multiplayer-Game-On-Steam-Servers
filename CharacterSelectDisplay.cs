...
public class CharacterSelectDisplay : NetworkBehaviour
{
    ...
    private void LockInServerRpc(ServerRpcParams serverRpcParams = default)
    {
        for (int i = 0; i < players.Count; i++)
        {
           ...
           // DataTransfer.Instance.clientId = players[i].ClientId;
          //  DataTransfer.Instance.characterId = players[i].CharacterId;
        ...
        for (int i = 0; i < players.Count; i++)
        {
            Debug.Log(players[i].ClientId + " " + players[i].CharacterId);
            DataTransfer.Instance.playersPrefabs.TryAdd(players[i].ClientId, players[i].CharacterId);
          //  DataTransfer.Instance.clientId.Add(players[i].ClientId);
          //  DataTransfer.Instance.characterId.Add(players[i].CharacterId);
        }
